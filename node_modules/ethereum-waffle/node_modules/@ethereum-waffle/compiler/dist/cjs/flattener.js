...
async function flattenProject(configPath) {
    await flattenAndSave(await (0, config_1.loadConfig)(configPath));
}
...
async function getContractDependency(config) {
    const resolver = (0, imports_fs_1.ImportsFsEngine)().addResolver(imports_fs_1.resolvers.BacktrackFsResolver(config.nodeModulesDirectory));
    const allContracts = (0, findInputs_1.findInputs)(config.sourceDirectory, (0, utils_1.getExtensionForCompilerType)(config));
    return Promise.all(allContracts.map(async (contract) => (0, imports_1.gatherSourcesAndCanonizeImports)([contract], '.', resolver)));
}
const fsOps = {
...
function normalizeSpdxLicenceIdentifiers(flattenContracts, contractName) {
  ...
  Solidity compiler does not allow more than one licence. Licence selected: ${uniqueLicences}
    ...
    return (0, utils_1.insert)(normalizedContract, firstLicence[0], firstLicence.index);
}
