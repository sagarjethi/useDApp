...
async function compileProject(configPath) {
    const partialConfig = await (0, config_1.loadConfig)(configPath);
    await compileAndSave(partialConfig);
    const config = (0, config_1.inputToConfig)(partialConfig);
    if (config.typechainEnabled) {
        await (0, generateTypes_1.generateTypes)(config);
    }
}
exports.compileProject = compileProject;
async function compileAndSave(input) {
    const config = (0, config_1.inputToConfig)(input);
    const output = await compile(config);
    await processOutput(output, config);
}
exports.compileAndSave = compileAndSave;
async function compile(input) {
    return newCompile((0, config_1.inputToConfig)(input));
}
exports.compile = compile;
async function newCompile(config) {
    const resolver = (0, imports_fs_1.ImportsFsEngine)().addResolver(
    // Backwards compatibility - change node_modules path
    imports_fs_1.resolvers.BacktrackFsResolver(config.nodeModulesDirectory));
    const sources = await (0, imports_1.gatherSources)((0, findInputs_1.findInputs)(config.sourceDirectory, (0, utils_1.getExtensionForCompilerType)(config)), '.', resolver);
    return (0, getCompileFunction_1.getCompileFunction)(config)(sources);
}
async function processOutput(output, config) {
    ...
    if (anyNonWarningErrors(output.errors)) {
        ...
        await (0, saveOutput_1.saveOutput)(output, config);
    }
}
