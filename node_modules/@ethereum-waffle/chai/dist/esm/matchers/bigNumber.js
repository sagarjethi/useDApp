...
function overwriteBigNumberCloseTo(_super, chaiUtils) {
    return function (...args) {
        ...
        if (BigNumber.isBigNumber(expected) || BigNumber.isBigNumber(actual) || BigNumber.isBigNumber(delta)) {
            this.assert(BigNumber.from(expected).sub(actual).abs().lte(delta), `Expected "${expected}" to be within ${delta} of ${actual}`, `Expected "${expected}" NOT to be within ${delta} of ${actual}`, `A number between ${BigNumber.from(actual).sub(delta)} and ${BigNumber.from(actual).sub(delta)}`, expected);
        }
        else {
            _super.apply(this, args);
        }
