...
function overwriteBigNumberCloseTo(_super, chaiUtils) {
    return function (...args) {
        ...
        if (ethers_1.BigNumber.isBigNumber(expected) || ethers_1.BigNumber.isBigNumber(actual) || ethers_1.BigNumber.isBigNumber(delta)) {
            this.assert(ethers_1.BigNumber.from(expected).sub(actual).abs().lte(delta), `Expected "${expected}" to be within ${delta} of ${actual}`, `Expected "${expected}" NOT to be within ${delta} of ${actual}`, `A number between ${ethers_1.BigNumber.from(actual).sub(delta)} and ${ethers_1.BigNumber.from(actual).sub(delta)}`, expected);
        }
        else {
            _super.apply(this, args);
        }
