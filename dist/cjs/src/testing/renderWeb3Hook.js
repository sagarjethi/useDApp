...
const renderWeb3Hook = async (hook, options) => {
    ...
    const defaultProvider = (options === null || options === void 0 ? void 0 : options.mockProvider) || new ethereum_waffle_1.MockProvider();
    ...
    if (Object.keys(readOnlyProviders).length === 0) {
        const defaultReadOnlyProvider = new ethereum_waffle_1.MockProvider();
    ...
    return {
        ...
        defaultProvider,
        mineBlock: async () => {
            await Promise.all([defaultProvider, ...Object.values(readOnlyProviders)].map((provider) => (0, utils_1.mineBlock)(provider)));
        },
        rerender,
        unmount,
