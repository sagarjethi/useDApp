...
const ethereum_waffle_2 = require("ethereum-waffle");
const abi_1 = require("../constants/abi");
const chai_1 = require("chai");
const crypto_1 = require("crypto");
(0, chai_1.use)(ethereum_waffle_2.solidity);
...
describe('useCall - three chains', () => {
    ...
    for (const chainId of chainIds) {
        const provider = new ethereum_waffle_1.MockProvider({ ganacheOptions: { chain: { chainId } } });
    ...
    beforeEach(async () => {
        for (const [, chain] of Object.entries(chains)) {
            chain.timestampContract = await (0, ethereum_waffle_2.deployContract)(chain.deployer, abi_1.timestampContractABI);
            chain.doublerContract = await (0, ethereum_waffle_2.deployContract)(chain.deployer, abi_1.doublerContractABI);
            chain.multicallAddress = (await (0, ethereum_waffle_2.deployContract)(chain.deployer, abi_1.MultiCall)).address;
            if (chain.mineBlock) {
